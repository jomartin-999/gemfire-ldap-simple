...
//    public static final String GF_LDAP_SSL_TRUSTSTORE_PASSWORD = "gf-ldap-ssl-truststore-password";
    ...
    public static Properties loadGFLDAPProperties() throws IOException {
        return loadPropertiesFromCommonLocations(GF_LDAP_SECURITY_PROPERTIES);
    }

    private static Properties loadPropertiesFromCommonLocations(String defaultFileName) throws IOException {
        Properties properties = new Properties();
        if (new File(defaultFileName).exists()) {
            log.debug("using current working directory properties");
