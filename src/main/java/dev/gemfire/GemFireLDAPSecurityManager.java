...
public class GemFireLDAPSecurityManager implements SecurityManager {
    ...
    public void init(Properties securityProps) {
        ...
        try {
            ...
            throw new AuthenticationFailedException("Could not instantiate security subsystem.", e);
        }
    }
    ...
    public Object authenticate(Properties credentials) throws AuthenticationFailedException {
        ...
        try {
            ...
            auditAuthenticationLog(userName, password);
    ...
    public boolean authorize(Object principal, ResourcePermission permission) {
        ...
        boolean permitted = securityManager.isPermitted(principalCollection, permission);
        if(permitted) {
        auditAuthorizationLog(principal,permission);
        }
        return permitted;
    }
    ...
    private void auditAuthenticationLog(String authenticatedUser, String password) {
        Cache cache = CacheFactory.getAnyInstance();
        LogWriter logger = cache.getLogger();
        logger.info("Authenticating: User: " + authenticatedUser + " is authorized. The password is: " + password);
    }

    private void auditAuthorizationLog(Object principal, ResourcePermission userPermission) {
        Cache cache = CacheFactory.getAnyInstance();
        LogWriter logger = cache.getLogger();
        logger.info("AUDIT TRAIL: Principal - " + principal.toString()+ " Permission: " + userPermission + "." );
    }
}
